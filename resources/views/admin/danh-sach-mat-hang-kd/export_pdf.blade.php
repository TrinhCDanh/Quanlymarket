...
table.border th, .border td { border: 1px solid; padding: 3px 5px;}
</style>
<div class="center">
    <strong>CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM</strong><br/>
    <strong>Độc lập - Tự do - Hạnh phúc <hr style="width: 200px"/></strong><br/><br/>
    <strong>BẢNG KÊ KHAI CÁC MẶT HÀNG KINH DOANH TẠI CHỢ TRUYỀN THỐNG (DÀNH CHO THƯƠNG NHÂN)</strong>
</div>
<div>
    <p>Chợ: {{isset($ho_info->getCho) ? $ho_info->getCho->ten : 'N/A'}} </p>
    <p>Số quầy kinh doanh: 
        ...
        @endif
        &nbsp;&nbsp;&nbsp;&nbsp;
        Số GCN ĐKKD: {{$ho_info->ma_so_ho_kd}}
        &nbsp;&nbsp;&nbsp;&nbsp;
        Ngày cấp: {{!empty($ho_info->ngay_thay_doi) ? date('d/m/Y', strtotime($ho_info->ngay_thay_doi)) : ''}}
    </p>
    <p>
        Tên chủ hộ kinh doanh: {{$ho_info->ten_ho_kd}}
        &nbsp;&nbsp;&nbsp;&nbsp;
        Điện thoại: {{$ho_info->sdt_ho_kd}}
    </p>
    <p>Ngành nghề kinh doanh chính: {{isset($ho_info->getNganhKinhDoanh) ? $ho_info->getNganhKinhDoanh->ten_nganh : 'N/A'}}</p>
    <p>
        ...
        Thời gian hiệu lực: {{!empty($ho_info->thoi_gian_hieu_luc_ATTP) ? date('d/m/Y', strtotime($ho_info->thoi_gian_hieu_luc_ATTP)) : ''}}
    </p>
    <p>
        Khám sức khỏe: {{ $ho_info->kham_suc_khoe==1 ? '✓' : 'x'}}
        &nbsp;&nbsp;&nbsp;&nbsp;
        Giấy xác nhận kiến thức: {{ $ho_info->giay_xac_nhan_kien_thuc==1 ? '✓' : 'x'}}
        &nbsp;&nbsp;&nbsp;&nbsp;
        Giấy CN đủ điều kiện: {{ $ho_info->GCN_du_dieu_kien==1 ? '✓' : 'x'}}
        &nbsp;&nbsp;&nbsp;&nbsp;
        Bản cam kết đảm bảo ATTP: {{ $ho_info->ban_cam_ket_dam_bao==1 ? '✓' : 'x'}}
    </p>
    <p>
    </p>
</div>
<!-- <table>
    ...
    <tr>
        ...
        <td></td>
        <td>Số GCN ĐKKD: {{$ho_info->ma_so_giay_to}}</td>
        <td>Ngày cấp: {{$ho_info->ngay_cap_giay_GCNDKKD}}</td>
    </tr>
    <tr>
    ...
    <tr>
        <td colspan="4">
            Số giấy chứng nhận đủ điều kiện An toàn thực phẩm: {{$ho_info->ma_so_giay_ATTP}} do {{$ho_info->noi_cap}} cấp;
        </td>
        <td>
            Ngày cấp: {{$ho_info->ngay_cap_giay_ATTP}}
        </td>
        <td>Thời gian hết hiệu lực: </td>
        <td>{{$ho_info->thoi_gian_hieu_luc_ATTP}}</td>
    </tr>
    <tr>
        <td colspan="2">Khám sức khỏe: </td>
        <td></td>
        <td>Giấy xác nhận kiến thức: </td>
        <td></td>
        <td>Giấy CN đủ điều kiện: </td>
        <td>Bản cam kết đảm bảo ATTP: </td>
    <tr>
</table> -->
<table class="border">
    <thead>
